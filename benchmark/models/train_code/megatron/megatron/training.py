...
def train_step(forward_step_func, data_iterator,
    ...
    if args.deepspeed and args.ds_pipeline_enabled:
        skipped_iter = 0
        num_zeros_in_grad = mpu.get_pipeline_model_parallel_world_siz
        assert isinstance(model[0], deepspeed.PipelineEngine)
        loss = model[0].train_batch(data_iter=data_iterator)
        grad_norm = model[0].get_global_grad_norm()
        return {'"lm_loss"' : loss}, skipped_iter, grad_norm, num_zeros_in_grad
...
def training_log(loss_dict, total_loss_dict, learning_rate, iteration,
    ...
    if iteration % args.log_interval == 0:
        ...
        forward_time=timers('forward-compute').elapsed()
        ...
        log_string = '{{"iteration": {:8d} ,'.format(
            iteration)
        log_string += ' "consumed_samples": {:12d} ,'.format(
            args.consumed_train_samples)
        log_string += ' "consumed_tokens": {:12d} ,'.format(
            args.consumed_train_tokens)
        log_string += ' "elapsed_time_per_iteration": {:.1f} ,'.format(
            elapsed_time_per_iteration * 1000.0)
        log_string += ' "learning_rate": {:.3E} ,'.format(learning_rate)
        log_string += ' "global_batch_size": {:5d} ,'.format(batch_size)
        ...
        log_string += ' "skipped_iterations": {:3d} ,'.format(
            total_loss_dict[skipped_iters_key])
        log_string += ' "nan_iterations": {:3d} ,'.format(
            total_loss_dict[nan_iters_key])
        log_string += ' "samples_per_second": {:.3f} ,'.format(samples_per_sec)
        log_string += ' "TFLOPs": {:.2f}'.format(tflops)
        log_string += ' "Forward": {:.5f}'.format( forward_time*1000)
        log_string += ' "Backward": {:.5f}'.format( (timers('backward-all').elapsed()-forward_time)*1000)
        log_string += ' "Optimizer": {:.5f}}}'.format( (timers('optimizer').elapsed())*1000)
        total_loss_dict[advanced_iters_key] = 0
        total_loss_dict[skipped_iters_key] = 0
        total_loss_dict[nan_iters_key] = 0
        print_rank_last(log_string,iteration)
    ...
    with torch.no_grad():
        ...
        while iteration < args.eval_iters:
            ...
            if args.deepspeed and args.ds_pipeline_enabled:
                ...
                loss_dicts = [{'"lm_loss"' : loss}] * get_num_microbatches()
            else:
...
def evaluate_and_print_results(prefix, forward_step_func,
    ...
    string = ' {{ "steps": {} ,'.format(prefix)
    for key in total_loss_dict:
        string += '"{}_value": {:.6E} , '.format(key, total_loss_dict[key].item())
        ppl = math.exp(min(20, total_loss_dict[key].item()))
        string += '"{}_PPL": {:.6E} }} '.format(key, ppl)
        if writer and is_last_rank():
