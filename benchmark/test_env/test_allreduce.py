...
def train(args):
    ...
    allreduce_message_size_total = 2*(tp_size-1)*bs*512/1024#allreduce_message_size_per_layer * 24
    if rank == 0:
    ...
    def trace_handler(prof):
        if rank == 0:
            ...
            print('**********')
            print('comm_coe_%d_%d_%d (MB/ms):'%(pp_size,tp_size,args.global_tp_consec), comm_coe)
            print('**********')
    ...
    with torch.profiler.profile(activities=[torch.profiler.ProfilerActivity.CUDA],
                                schedule=torch.profiler.schedule(wait=0,warmup=5,active=10),
