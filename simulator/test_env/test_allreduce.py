...
def train(args):
    ...
    allreduce_message_size_per_layer = 2 * \
        (tp_size-1)/tp_size*(bs*512*1024*2*4/1024/1024)
    # allreduce_message_size_per_layer * 24
    allreduce_message_size_total = 2*(tp_size-1)*bs*512/1024
    if rank == 0:
        print('Strategy: %d_%d_%d' % (pp_size, tp_size, args.global_tp_consec))
        print('[allreduce_message_size]: per_layer %d MB, total %d MB' %
              (allreduce_message_size_per_layer, allreduce_message_size_total))
    ...
    def trace_handler(prof):
        if rank == 0:
            ...
            print(allreduce_message_size_total, allreduce_time_24_layer)
            print('**********')
            print('comm_coe_%d_%d_%d (MB/ms):' %
                  (pp_size, tp_size, args.global_tp_consec), comm_coe)
            print('**********')

            comm_type = 'allreduce'
            gpu_num = world_size * pp_size
            env_config_path = './env_params.json'
            config = read_json_config(env_config_path) if os.path.exists(
                env_config_path) else dict()
            key = 'env_params'
            config[key] = comm_coe
            write_json_config(config, env_config_path)
            print('Already written comm_coe_%s into env config file %s!' %
                  (key, env_config_path))

    with torch.profiler.profile(activities=[torch.profiler.ProfilerActivity.CUDA],
                                schedule=torch.profiler.schedule(
                                    wait=0, warmup=5, active=10),
