...
def get_node_unrequested(
    ...
    """
    ret = {
        key: _discretize_resource(key, val)
        for key, val in node.status.allocatable.items()
    }
    for pod in pods:
        if pod.spec.node_name != node.metadata.name or pod.status.phase in [
            "Succeeded",
            "Failed",
        ]:
            continue
    ...
    resources = {key: val for key, val in ret.items() if val > 0}
    # hack bandwidth setting
...
def set_default_resources(pod_spec: dict) -> dict:
    """
    Set the default resources for a given MorphlingJob spec.
...
def _discretize_resource(name, value):
    ...
    factor = 1 if name == "cpu" else 1  # Convert CPU to mCPU.
