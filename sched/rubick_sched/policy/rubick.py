import collections
from rubick_sched.model import Model
import copy
import math
rsc_type = ["nvidia.com/gpu", "cpu", "bandwidth"]
empty_rsc = {"nvidia.com/gpu": 0, "cpu": 0, "bandwidth": 0}
...
class RubickPolicy(object):
    ...
    def shrink(self, job_infos, targeted_job, targeted_nodes, node_idx, total_rsc, execution_plan, job_alloc):
        ...
        if job_infos[targeted_job].job_gpu_demand > 8:
            ...
            bw = 0
        if free_rsc["nvidia.com/gpu"] < 0:
            job_throughput_base, job_strategy, job_rsc_alloc = -1, None, None
        else:
            job_throughput_base, job_strategy, job_rsc_alloc = job_infos[targeted_job].rsc_matrix[free_rsc["nvidia.com/gpu"]][free_rsc["cpu"]//(
                1e5 if free_rsc["nvidia.com/gpu"] <= 0 else free_rsc["nvidia.com/gpu"])][bw]
        ...
        while not can_allocated and not should_exit:
            ...
            while delta_addition:
                rsc_delta[0]["nvidia.com/gpu"] += 1
                rsc_delta[1]["cpu"] = rsc_delta[1]["cpu"] if rsc_delta[1]["nvidia.com/gpu"] == 0 else min(
                    rsc_delta[1]["cpu"]+rsc_delta[1]["nvidia.com/gpu"], total_rsc["cpu"])
                if job_infos[targeted_job].job_gpu_demand > 8:
                ...
                if rsc_delta[0]["nvidia.com/gpu"] >= total_rsc["nvidia.com/gpu"] and (rsc_delta[1]["cpu"] >= total_rsc["cpu"] or rsc_delta[1]["nvidia.com/gpu"] == 0):
                    delta_addition = False
                    should_exit = True
                    break
                rsc_delta.sort(key=lambda r: job_infos[targeted_job].rsc_matrix[r["nvidia.com/gpu"]][r["cpu"]//(
                    1e5 if r["nvidia.com/gpu"] == 0 else r["nvidia.com/gpu"])][r["bandwidth"]][0], reverse=True)
                # Targeted job and resource
                for rsc_delta_target in rsc_delta:
                    job_throughput, job_strategy, job_rsc_alloc = job_infos[targeted_job].rsc_matrix[rsc_delta_target["nvidia.com/gpu"]][rsc_delta_target["cpu"]//(
                        1e5 if rsc_delta_target["nvidia.com/gpu"] == 0 else rsc_delta_target["nvidia.com/gpu"])][rsc_delta_target["bandwidth"]]
                    if job_throughput == -1:
        ...
        for rsc in rsc_type:
            ...
            if rsc == "cpu":
                execution_plan[rsc][targeted_job].extend(
                    [node_idx]*targeted_job_rsc[rsc]*targeted_job_rsc["nvidia.com/gpu"])
            else:
                execution_plan[rsc][targeted_job].extend(
    ...
    def clear_bw(self, execution_plan):
        node_idx = set()
        for j in execution_plan["nvidia.com/gpu"]:
            node_idx = set(
                execution_plan["nvidia.com/gpu"][j]) | set(execution_plan["cpu"][j])
    ...
    def sort_slope_v(self, slope_v, shrink_rsc, job_infos, targeted_job, free_rsc, targeted_nodes):
        ...
        for k, v in slope_v.items():
            ...
            job_throughput, targeted_job_strategy[k], targeted_job_rsc_alloc[k] = job_infos[targeted_job].rsc_matrix[updated_rsc_matrix["nvidia.com/gpu"]][updated_rsc_matrix["cpu"]//(
                1e5 if updated_rsc_matrix["nvidia.com/gpu"] == 0 else updated_rsc_matrix["nvidia.com/gpu"])][updated_rsc_matrix["bandwidth"]]
    ...
    def shrink_delta_rsc(self, job_infos, rsc_delta_target, targeted_nodes, job_alloc, targeted_job, free_rsc):
        ...
        rsc_shrink = {}
        shrink_rsc = {}
        if rsc_delta_target["nvidia.com/gpu"] > 0:
            for job in targeted_nodes:
                rsc = [job_alloc[job]["nvidia.com/gpu"], job_alloc[job]["cpu"] //
                       job_alloc[job]["nvidia.com/gpu"], job_alloc[job]["bandwidth"]]
                num_restarts = self.judge_restart(job_infos[job])
                slope_matrix[job], shrink_rsc[job] = self.cal_slope(
                    "nvidia.com/gpu", rsc[0], rsc[1], rsc[2], job_infos[job].rsc_matrix, rsc_delta_target["nvidia.com/gpu"], job_infos[job].local_bsz, num_restarts)
            ...
            if shrink_rsc[shrink_job_name]["nvidia.com/gpu"] <= 0:
                return slope_down, rsc_shrink, None, None
            ...
            job_alloc[shrink_job_name]["nvidia.com/gpu"] -= shrink_rsc[shrink_job_name]["nvidia.com/gpu"]
            ...
            for job in targeted_nodes:
                if job_infos[job].is_distributed:
                    continue
                num_restarts = self.judge_restart(job_infos[job])
                rsc = [targeted_nodes[job]["nvidia.com/gpu"], targeted_nodes[job]["cpu"] //
                       targeted_nodes[job]["nvidia.com/gpu"], targeted_nodes[job]["bandwidth"]]
                slope_matrix[job], shrink_rsc[job] = self.cal_slope(
            ...
            targeted_nodes[shrink_job_name]["nvidia.com/gpu"] -= shrink_rsc[shrink_job_name]["nvidia.com/gpu"]
            ...
            for job in targeted_nodes:
                ...
                rsc = [targeted_nodes[job]["nvidia.com/gpu"], targeted_nodes[job]["cpu"] //
                       targeted_nodes[job]["nvidia.com/gpu"], targeted_nodes[job]["bandwidth"]]
                slope_matrix[job], shrink_rsc[job] = self.cal_slope(
                    "nvidia.com/gpu", rsc[0], rsc[1], rsc[2], job_infos[job].rsc_matrix, rsc_delta_target["bandwidth"], job_infos[job].local_bsz, num_restarts)
    ...
    def expand_delta_rsc(self, job_infos, rsc_delta_target, targeted_nodes):
        ...
        rsc_expand = {}
        if rsc_delta_target["nvidia.com/gpu"] > 0:
            delta_gpu = rsc_delta_target["nvidia.com/gpu"]
            while sum(rsc_expand[j]["nvidia.com/gpu"] for j in rsc_expand) < delta_gpu:
                for job in targeted_nodes:
                ...
                for item in slope_v:
                    ...
                    if expand_rsc[expand_job_name]["nvidia.com/gpu"] <= 0:
                        return expand_job, rsc_expand
                    if -item[1] > 1e-2:
                        ...
                        targeted_nodes[expand_job_name]["nvidia.com/gpu"] += expand_rsc[expand_job_name]["nvidia.com/gpu"]
                        rsc_delta_target["nvidia.com/gpu"] -= expand_rsc[expand_job_name]["nvidia.com/gpu"]
                        break
    ...
    def cal_slope(self, t_rsc_type, gpu_num, cpu_num, bandwidth, rsc_matrix, delta_rsc, local_bsz, restarts):
        ...
        if t_rsc_type == "nvidia.com/gpu":
            ...
            rsc_shrink["nvidia.com/gpu"] = abs(
                original_rsc_matrix[2]["nvidia.com/gpu"]-changed_rsc_matrix[2]["nvidia.com/gpu"])
            rsc_shrink["cpu"] = abs(original_rsc_matrix[2]["nvidia.com/gpu"]*original_rsc_matrix[2]
                                    ["cpu"]-changed_rsc_matrix[2]["nvidia.com/gpu"]*changed_rsc_matrix[2]["cpu"])
    ...
    def _max_throughput_gpus(self, norm_demand_vector, num_gpus, free_rsc, single_node):
        rsc_alloc = {}
        free_gpu = sorted(
            free_rsc["nvidia.com/gpu"].items(), key=lambda x: x[1], reverse=True)
        for f_g in free_gpu:
            gpu_node = f_g[0]
            if single_node:
                rsc_alloc = {}
            rsc_alloc[gpu_node] = 0
            gpus = free_rsc["nvidia.com/gpu"][gpu_node]
            for gpu in range(0, gpus):
                if self._fits_in_nodes(gpu_node, norm_demand_vector, free_rsc):
                    rsc_alloc[gpu_node] += 1
                    free_rsc["nvidia.com/gpu"][gpu_node] -= 1
