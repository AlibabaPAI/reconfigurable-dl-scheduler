...
class SynergyPolicy(object):
    ...
    def optimize(self, job_infos, node_infos, prev_execution_plan, node_template):
        ...
        for rsc in rsc_type:
            execution_plan[rsc] = {
                k: v for k, v in prev_execution_plan[rsc].items() if k in job_infos and job_infos[k].lease_time < lease_allocation_time}
        ...
        jobs_gpu = {}
        num_free_gpus = sum(free_rsc["nvidia.com/gpu"][node_idx]
        ...
        for job_v in jobs_this_round:
            ...
            job_gpu_deficit = job_infos[job].job_demand_vector[0]
            if job_gpu_deficit > 0 and sum(free_rsc["nvidia.com/gpu"][node_idx] for node_idx in free_rsc["nvidia.com/gpu"]) >= job_gpu_deficit:
    ...
    def _top_synergy_gpus(self, norm_demand_vector, num_gpus, free_rsc):
        ...
        pq = [(num, node_idx)
              for node_idx, num in free_rsc["nvidia.com/gpu"].items()]
        heapq.heapify(pq)
        ...
        while sum(rsc_addition[node_idx] for node_idx in rsc_addition) < num_gpus and len(pq) > 0:
            ...
            if self._fits_in_server(gpu_node, norm_demand_vector, copy.deepcopy(free_rsc)):
                gpus = free_rsc["nvidia.com/gpu"][gpu_node]-1
    ...
    def _make_fair_share(self, job_infos, job, demand_vec, node_infos):
        ...
        if self.per_server_size_fair == None:
            per_server_size_fair = [node_infos[0].resources[rsc] /
                                    node_infos[0].resources["nvidia.com/gpu"] for rsc in rsc_type]
    ...
    def _reallocate_peer(self, demand_vec, avail_vec, node_idx, execution_plan):
        ...
        job_list.sort(key=lambda x: (len(set(execution_plan["nvidia.com/gpu"][x])), -sum(len(execution_plan["cpu"][x]))/sum(len(
            execution_plan["nvidia.com/gpu"][x])), -sum(len(execution_plan["memory"][x]))/sum(len(execution_plan["nvidia.com/gpu"][x]))))

        for job in job_list:
            job_gpus_this_server = execution_plan["nvidia.com/gpu"][job].count(
                node_idx)
            job_fair = [
            ...
            job_gpu_share = job_gpus_this_server / \
                job_alloc_vector["nvidia.com/gpu"]
