...
import logging
import math
import rubick_sched.k8s_templates as templates
import rubick_sched.config as config
import time
from datetime import datetime, timezone
from prometheus_client import Counter, Summary
import collections
from rubick_sched.resources import set_default_resources
from rubick_sched.utils import patch_job_status
import os
...
class JobsController(object):
    ...
    def __init__(self):
        ...
        self._custom_resource = ("rubick.alibaba.com", "v1",
                                 "", "rubickjobs")
        ...
        async with kubernetes.watch.Watch() as watch:
            while True:
                async for event in watch.stream(
                        ...
                        *self._custom_resource, timeout_seconds=10):
        ...
        async with kubernetes.watch.Watch() as watch:
            while True:
                async for event in watch.stream(
                        ...
                        label_selector="rubick/job", timeout_seconds=60):
                    pod = event["object"]
                    job_name = pod.metadata.labels["rubick/job"]
    ...
    async def _sync_job(self, namespace, job_name):
        ...
        if (completion_status := self._detect_completion(pyJob, pods, job["status"]["remainingSteps"])):
            ...
            if "nvidia.com/gpu" in allocation and allocation["nvidia.com/gpu"] and not pods:
            ...
            if (self._count_scheduled_pods(pods) != replicas and
                    self._detect_restart(pyJob, pods, copy.deepcopy(allocation))) or not allocation or not allocation["nvidia.com/gpu"]:
                ...
                await self._create_pytorch_job(
                    job["metadata"], job["spec"]["application"],
                    job["spec"]["template"], copy.deepcopy(allocation),
                    job["status"]["group"], cpuID,
                    job["spec"]["localBsz"], job["spec"]["workDir"], job["status"]["remainingSteps"])
                job["status"]["lease_time"] = time.time()
            elif self._count_gpus(pods) != replicas and (datetime.now()-datetime.strptime(pyJob["metadata"]["creationTimestamp"], "%Y-%m-%dT%H:%M:%SZ")).total_seconds() > 90:
            ...
            if self._detect_restart(pyJob, pods, copy.deepcopy(allocation)) or \
                    not pods:
            ...
            if pods:
                remainingSteps = await self._delete_pyJob(pyJob, pods, job["status"]["remainingSteps"])
                job["status"]["remainingSteps"] = remainingSteps
            else:
        ...
        # Apply changes to RubickDLJob status.
        patch = {"status": {k: v for k, v in job["status"].maps[0].items()
                            if v != job["status"].maps[1].get(k)}}
        if patch["status"]:
            LOG.info("Patch RubickDLJob %s: %s", job_name, patch)
    ...
    def _count_gpus(self, pods):
        count = 0
        for pod in pods:
            count += int(pod.spec.containers[0]
    ...
    async def _get_job_and_pyJob(self, namespace, name):
        try:
            job = await self._objs_api.get_namespaced_custom_object(
                "rubick.alibaba.com", "v1", namespace, "rubickjobs", name)
        ...
        try:
            pyJob = await self._objs_api.list_namespaced_custom_object(
                "kubeflow.org", "v1", namespace, "pytorchjobs", label_selector=f"rubick/job={name}")
        except kubernetes.client.rest.ApiException as exc:
            if exc.status == 404:
        ...
        if len(pyJob["items"]) > 0:
            ...
            pods = await self._core_api.list_namespaced_pod(namespace, label_selector=f"training.kubeflow.org/job-name={pyJobName}")
            pods = pods.items
        else:
            pyJob = None
            pods = []
    ...
    async def _validate_pyJob(self, job, pyJob, pods):
        ...
        try:
            ...
            patch_status["message"] = \
                f"invalid annotations for pyJob {tmp_name}"
    ...
    def _detect_completion(self, pyJob, pods, remaining_steps):
        ...
        if os.path.exists("/mnt/nasdata/ckpt/"+pyJob["metadata"]["name"]+".log"):
            ...
            try:
                ...
                LOG.info("Can not read lines %s", "/mnt/nasdata/ckpt/" +
                         pyJob["metadata"]["name"]+".log")
                last_line = 1
    ...
    def _detect_restart(self, pyJob, pods, allocation):
        ...
        if self._count_gpus(pods) != len(gpuAlloc) and len(pods) != 0 and (datetime.now()-datetime.strptime(pyJob["metadata"]["creationTimestamp"], "%Y-%m-%dT%H:%M:%SZ")).total_seconds() > 90:
            return True
        for pod in pods:
            if pod.spec.node_name:
                n = pod.spec.node_name
                for i in range(0, int(pod.spec.containers[0].resources.limits["nvidia.com/gpu"])):
                    if n in gpuAlloc:
                        gpuAlloc.remove(n)
                    else:
                        return True
    ...
    async def _delete_pyJob(self, pyJob, pods, remaining_steps=0):
        ...
        if "deletion_timestamp" not in pyJob["metadata"]:
            results.append(self._objs_api.delete_namespaced_custom_object(
                group="kubeflow.org", version="v1", namespace=pyJob["metadata"]["namespace"], plural="pytorchjobs", name=pyJob["metadata"]["name"], body=kubernetes.client.V1DeleteOptions()))
            names.append(pyJob["metadata"]["name"])
        if results:
            LOG.info(f"Deleting {names}")
            await asyncio.gather(*results, return_exceptions=True)
            if remaining_steps != 0 and os.path.exists("/mnt/nasdata/ckpt/"+pyJob["metadata"]["name"]+".log"):
                file = open("/mnt/nasdata/ckpt/" +
                            pyJob["metadata"]["name"]+".log")
                try:
                    lines = file.readlines()
                    last_line = lines[-1]
    ...
    async def _create_pytorch_job(self, job_metadata, application, pod_template, allocation, group, cpuIDs, local_bsz, workDir, steps):
        ...
        pyJob["metadata"]["ownerReferences"] = \
            templates.owner_reference_template(
                job_metadata["namespace"], job_metadata["name"],
                job_metadata["uid"], kind="RubickJob")
        pyJob["metadata"].setdefault("labels", {})
        pyJob["metadata"]["labels"]["rubick/job"] = job_metadata["name"]
        ...
        if allocation["strategy"][2] == "zero-offload" and cpuIDs != {}:
            ...
            pyJob["metadata"]["annotations"]["cpuWorkerID"] = json.dumps(
                cpuWorkerAlloc)
        ...
        pyJob["spec"]["pytorchReplicaSpecs"]["Master"]["restartPolicy"] = pyJob[
            "spec"]["pytorchReplicaSpecs"]["Worker"]["restartPolicy"] = "OnFailure"
        ...
        for idx, container in enumerate(pod_template["spec"]["containers"]):
            ...
            if application in ["vit", "roberta", "swin"]:
                if allocation["strategy"][2] == "zero-offload":
                    container["args"][0] += "  --deepspeed " + \
                        workDir+"config/zero_opt.json "
                    container["env"].append({
                    ...
                    container["args"][0] += "  --deepspeed " + \
                        workDir+"config/zero_2_opt.json "
                ...
                if allocation["strategy"][0] == "3D":
                    ...
                    if len(allocation["nvidia.com/gpu"]) == 2 and application in ["bert", "gpt2"]:
                        micro_bsz = 1
                elif allocation["strategy"][1] == "gc":
                    container["args"] = container["args"][:1]
        ...
        pyJob["spec"]["pytorchReplicaSpecs"]["Master"]["template"] = pyJob["spec"]["pytorchReplicaSpecs"]["Worker"]["template"] = pod_template
